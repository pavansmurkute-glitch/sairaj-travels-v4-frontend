...
const AdminUsers = () => {
  ...
  const loadUsers = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://sairaj-travels-v4-backend.onrender.com'}/api/admin/users`);
      if (response.ok) {
  ...
  const loadRoles = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://sairaj-travels-v4-backend.onrender.com'}/api/admin/roles/active`);
      if (response.ok) {
        const data = await response.json();
        setRoles(data);
      } else {
  ...
  const handleToggleActive = async (userId, currentStatus) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://sairaj-travels-v4-backend.onrender.com'}/api/admin/users/${userId}/${action}`, {
        method: 'PATCH'
      });

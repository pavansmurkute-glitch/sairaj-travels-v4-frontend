            ...
            {photos.map((photo) => (
              <div key={photo.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="aspect-video bg-gray-200 rounded-lg mb-3 flex items-center justify-center">
                  {photo.imagePath ? (
                    <img
                      src={photo.imagePath.startsWith('http') ? photo.imagePath : `${IMAGE_BASE_URL}${photo.imagePath}`}
                      alt={photo.title}
            ...
            {filteredGallery.map((item) => (
              <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                <div className="relative">
                  {item.imagePath ? (
                    <img
                      src={item.imagePath.startsWith('http') ? item.imagePath : `${IMAGE_BASE_URL}${item.imagePath}`}
                      alt={item.title}
